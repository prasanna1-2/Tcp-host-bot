...
async def lag_team_loop(team_code, key, iv, region):
    ...
    while lag_running:
        ...
        # ২০ সেকেন্ড হয়ে গেলে লুপ বন্ধ হবে
...
[FF0000]╠══════════╣
[FFD700]⚡ OWNER : [FFFFFF]MR KALPHA    
[FFD700]✨ Name on instagram : [FFFFFF] MR KALPHA❤️  
[FF0000]╚══════════╝
[FFD700]✨ Developer —͟͞͞ </> MR KALPHA ❄️  ⚡
"""
...
"""
                        ...
                        if inPuTMsG.strip() == '/stop_auto':
                            ...
                            if auto_start_task and not auto_start_task.done():
                                stop_auto = True # লুপ থামানোর সিগন্যাল
                                auto_start_running = False
                                auto_start_task.cancel() # টাস্কটি ফোর্স করে বন্ধ করা
                                success_msg = f"[B][C][00FF00]✅ SUCCESS! Auto start bot stopped successfully!\n"
                                await safe_send_message(response.Data.chat_type, success_msg, uid, chat_id, key, iv)
                            else:
                                error_msg = f"[B][C][FF0000]❌ ERROR! No active auto start to stop!\n"
                                await safe_send_message(response.Data.chat_type, error_msg, uid, chat_id, key, iv)
...
# IMPROVED TREE-STYLE HELP MENU SYSTEM (Commands in their original menus) 🌳
                        if inPuTMsG.strip().lower() in ("help", "/help", "menu", "/menu", "commands"):
                            ...
                            # Header
                            header = f"[b][c]{get_random_color()}Hey User Welcome To mr kalpha ˣ ʙᴏᴛ"
...
[00FFFA]║
[FFFF00]║ 👤 Developer    :: [FF1493] MR KALPHA
...
async def MaiiiinE():
    Uid , Pw = '4271525990','PRITAM-3588_ZVK8H_BY_SPIDEERIO_GAMING_LVLK1'
...
def handle_keyboard_interrupt(signum, frame):
    ...
    print("👋 Thanks for using MR KALPHA BOT")
    sys.exit(0)
